...
public class DBUtils {
	...
	/**
	 * @param criteria
	 *            : pair data of column and value
	 * @param is_union
	 *            : if yes, using OR operator. Otherwise, using AND operator in
	 *            WHERE clause
	 * @return
	 */
	...
	public static List<Trade> GetTradeByCriteria(HashMap<String, String> criteria, String field_name) {
		Set<Entry<String, String>> entrySet = criteria.entrySet();
		int size = entrySet.size();
		int i = 1;
		String query = new String("SELECT NB,instrument,currency,portfolio,trn_fmly,trn_grp,trn_type,trn_status,field,issue_id FROM trade WHERE "
				+ "not exists(SELECT field FROM trade WHERE ($KEY$) and field = '$FIELD$') and  ($KEY$)"
				+ " union " + "SELECT NB,instrument,currency,portfolio,trn_fmly,trn_grp,trn_type,trn_status,field,issue_id FROM Trade WHERE ($KEY$) and field = '$FIELD$'");
		
		StringBuilder cr = new StringBuilder();
		for (Entry entry : entrySet) {

			if (i++ < size)
				cr.append(entry.getKey() + " = '" + entry.getValue() + "' AND ");
			else
				cr.append(entry.getKey() + " = '" + entry.getValue() + "'");

		}
		query = query.replace("$KEY$", cr.toString());
		query = query.replace("$FIELD$", field_name);
		HbnTradeDao hbn = HbnTradeDao.getInstance();
		List<Trade> trades = hbn.getTradeByCriteria(query);
		return trades;
	}
	...
	public static int insertTrades(List<Trade> trades) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			...
			sb.append(String.format("(%d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d), \n",
					trade.getId().getNb(), trade.getInstrument(), trade.getCurrency(), trade.getPortfolio(),
					trade.getTrnFmly(), trade.getTrnGrp(), trade.getTrnType(), trade.getTrnStatus(),
					trade.getId().getField(), trade.getIssue().getId()));
			if ((i > 0 && i % 10000 == 0) || i == size - 1) {

				sb.insert(0, query);
				sb.deleteCharAt(sb.lastIndexOf(","))
						.append("ON DUPLICATE KEY UPDATE NB=VALUES(NB), field=VALUES(field)");
