...
public abstract class AbstractHbnDao<T extends Object> implements Dao<T> {
	...
	public void deleteAll() {
		...
		try {
			...
			session.createQuery("delete " + getDomainClassName()).executeUpdate();
			tx.commit();
		} catch (Exception e) {
			e.printStackTrace();
			tx.rollback();
	...
	// Open transaction for load (Exception in thread "main"
	// org.hibernate.HibernateException: load is not valid without active
	// transaction)
